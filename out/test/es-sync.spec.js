...
describe('AMQP Elasticsearch bulk sync', () => {
    ...
    beforeEach(() => {
        ...
        const channelWrapper = amqpConnection.createChannel({
            setup: channel => {
                return Promise.all([channel.assertExchange('change.events', 'topic'), channel.assertQueue('es.sync'), channel.purgeQueue('es.sync'), channel.bindQueue('es.sync', 'change.events', 'trackingData.insert')]);
            }
        });
    ...
    describe('AMQP rxjs', () => {
        ...
        it('feeds amqp messages with associated channel information and an ack/nack shorthands into an observer', done => {
            ...
            const subscription = esSync.esQueueObservable(global.amqpConnection).subscribe(event => {
    ...
    describe('End to End', () => {
        ...
        beforeEach(() => {
            return Promise.resolve().then(apiStart).then(clearData).then(createEquipment).then(esSetup.deleteIndex).then(esSetup.putIndex).then(esSetup.putMapping);

            function apiStart() {
                return Api.start(global.amqpConnection, config).then(server => {
        ...
        it('should ack the messages', done => {
            ...
            postTrackingData(config.bufferCount).then(trackingData => {
                ...
                const esQueueObservable = esSync.esQueueObservable(amqpConnection).map(event => {
                ...
                esSync.pipeline(esQueueObservable).subscribe(events => {
        ...
        it('should retry the pipeline on failure', done => {
            ...
            postTrackingData(config.bufferCount).then(trackingData => {
                ...
                const subscription = esSync.pipeline(esSync.esQueueObservable(amqpConnection)).subscribe(() => verifyResultsInEsAndDispose(trackingData, subscription).then(done), done);
            });
        });
        ...
        it('should gracefully recover when the AMQP connection fails before messages are ack\'ed', done => {
            ...
            postTrackingData(config.bufferCount).then(trackingData => {
                ...
                const subscription = esSync.pipeline(esSync.esQueueObservable(amqpConnection)).skip(2) // ack on a broken channel/connection doesn't result in an error so skip the first observed value batches
                .subscribe(() => verifyResultsInEsAndDispose(trackingData, subscription).then(done), done);
            });
        });
        ...
        it('should be able to cope with 1000 trackingData messages under 6 secs', done => {
            ...
            postTrackingData(docs).then(() => {
                const begin = new Date();

                const subscription = esSync.pipeline(esSync.esQueueObservable(amqpConnection)).bufferWithCount(docs / config.bufferCount).subscribe(events => {
                    const end = new Date();
        ...
        function verifyResultsInEsAndDispose(trackingData, subscription) {
            return Promise.all(lookupTrackingDataInEs(trackingData)).then(trackingDataFromEs => {
        ...
        function postTrackingData(maxRange) {
            ...
            return Promise.map(trackingData(maxRange), post, { concurrency: 10 });
...
//# sourceMappingURL=es-sync.spec.js.map