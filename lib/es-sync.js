                    ...
                    .spread((res, body)=> {
                        return _.map((trackingDataItem)=> {
                            if (isFunctionalError(trackingDataItem, body.included)) {
                                return sendChannel.sendToQueue('es-sync-error-queue', new Buffer(JSON.stringify(trackingDataItem)))
                            } else {
                                const associatedEvent = _.find(['source.json.data._id', trackingDataItem.id])(events)
                                return _.merge(associatedEvent, {result: {data: trackingDataItem, included: body.included}})
                            }
                        })(body.data)
                    })
            })
