...
function start(amqpConnection, config) {
    return esBulkSyncPipeline(config, esQueueToObserver(amqpConnection), fetchTrackingDataComposite(config), syncBufferedToEs(config))
}
...
function esQueueToObserver(amqpConnection) {

    return RxAmqp.amqpChannelObserver(amqpConnection, setupChannel)

    function setupChannel(observer) {
...
module.exports = {start, esBulkSyncPipeline, esQueueToObserver, fetchTrackingDataComposite, syncBufferedToEs}
