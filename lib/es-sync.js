...
function bootstrap(bufferThreshold) {
    return esBulkSyncPipeline(queueToObserver, bufferThreshold, fetchAndDenormalizeTrackingData, syncBufferedToEs)
}

function esBulkSyncPipeline(queueToObserver, bufferThreshold, fetchAndDenormalizeTrackingData, syncBufferedToEs) {
    return queueToObserver()
        .bufferWithTimeOrCount(300000, bufferThreshold)
        .map((events)=> {

            return Rx.Observable.defer(()=> {
                const eventsWithSourceAndResults = _.map(events, (event)=> {
                    return {
                        source: event,
...
module.exports = {bootstrap, esBulkSyncPipeline, queueToObserver, fetchAndDenormalizeTrackingData, syncBufferedToEs}