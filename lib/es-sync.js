...
module.exports = (config, amqpConnection) => {
            ...
            return Promise.resolve().then(()=> {
                return $http.get({
                    ...
                    .catch((e)=> {
                        ...
                        console.error(e) // todo check what happens if a runtime error is raised here in catch
    ...
    function calculateExpiration(headers) {
        if (headers["x-death"]) {
            const candidateExpiration = (headers["x-death"][0]["original-expiration"] * 3)
            return (candidateExpiration > 10000 ) ? 10000 : candidateExpiration
        } else {
            return 1000
        }
    }
