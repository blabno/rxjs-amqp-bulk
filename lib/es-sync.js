            ...
            function setup(channelWrapper, observer) {
                return channelWrapper.addSetup((channel)=>{
                    return Promise.all([
                        ...
                        channel.consume('es.sync.q', (msg)=> observer.onNext({msg, channel: channelWrapper}))
                    ])
                })
            }
        },
            ...
            return $http.get({
                ...
                .spread((res, body)=> {
                    if (!_.find({type: 'dealers'}, body.included)) {
                        return event.channel.sendToQueue('es.sync.dlq', new Buffer(JSON.stringify(trackingData)))
                            .then(()=> {
                                return {
                                    source: event
                                }
