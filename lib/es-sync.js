...
function start(amqpConnection, config) {
    return esBulkSyncPipeline(config, esQueueConsumeObservable(amqpConnection, config), fetchTrackingDataComposite(config), syncBufferedToEs(config))
}
...
function esQueueConsumeObservable(amqpConnection, config) {
    ...
    function setupChannel(consumeHandler) {
        return (channel) => Promise.all([
            channel.prefetch(config.bufferCount),
