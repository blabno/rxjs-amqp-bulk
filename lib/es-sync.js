    ...
    return {
        ...
        fetchTrackingDataComposite(events) {
            ...
            return Promise.resolve().then(()=> {
                const allTrackingDataIds = _.map('json.data._id')(events)
                const trackingDataIdsChunks = _.chunk(config.apiFetchSize)(allTrackingDataIds)

                return Promise.all(_.map(invokeAndProcessTrackingDataGet)(trackingDataIdsChunks)).then(_.flatten)
        ...
        ack(events) {
            ...
            const trackingIds = _.map('json.data._id', (events));
            console.log(`ack for messages with deliveryTag ${deliveryTags} and trackingData id ${trackingIds}`)
            return Promise.all(_.map(ackEvent)(events))
        },

        sendToRetryQueue(events) {
            console.log(`forwarding trackingData msgs to the retry queue. ids : ${_.map('json.data._id', events)}`)
