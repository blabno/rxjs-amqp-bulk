...
module.exports = (config, amqpConnection)=> {
    ...
    return {
        ...
        sendToRetryQueue(event) {
            ...
            return incrementExpiryAndSendToRetry(event).then(()=> false) // returning false on a caught failure, this facilitates testing
        }

    }

    function incrementExpiryAndSendToRetry(event) {
        const msg = event.msg
